...
public class GameManager : Singleton<GameManager>
{
    ...
    // UI 자식 객체들을 보존할 리스트
    private List<Transform> uiChildren = new();
    ...
    protected override void Awake()
    {
        ...
        // 씬 로드 후 UI 자식 객체를 옮기기 위한 이벤트 구독
        SceneManager.sceneLoaded += OnSceneLoaded;
    }

    private void OnDestroy()
    {
        // 씬 로드 후 UI 자식 객체 옮기는 이벤트 구독 해제
        SceneManager.sceneLoaded -= OnSceneLoaded;
    }

    // 씬 로드 완료 후 UI 자식들을 새 Canvas로 옮기기
    private void OnSceneLoaded(Scene scene, LoadSceneMode mode)
    {
        // 새로 로드된 씬에서 Canvas 찾기
        var newCanvas = GameObject.Find("Canvas")?.transform;
        if (newCanvas == null) return;

        // 기존 UI 자식들을 새 Canvas로 이동
        foreach (var child in uiChildren)
        {
            if (child != null)
                child.SetParent(newCanvas);
        }
    ...
    // Canvas의 자식들을 DontDestroyOnLoad 상태로 보존
    public void PreserveUIChildren()
    {
        ...
        // Canvas의 자식 객체들을 DontDestroyOnLoad로 보존
        foreach (Transform child in canvas)
        {
            uiChildren.Add(child); // 리스트에 자식 추가
            child.SetParent(null); // 부모에서 분리
            DontDestroyOnLoad(child.gameObject); // 씬 전환 시에도 보존
        }
    }
