...
public class VendigoPatorlState : VendigoBaseState
{
    ...
    private void SetNewWanderTarget()
    {
        ...
        Vector3 candidate = vendigo.transform.position + new Vector3(rand.x, 0f, rand.y);

        NavMeshHit hit;
        if (NavMesh.SamplePosition(candidate, out hit, WANDER_RADIUS, NavMesh.AllAreas))
        {
            // 현재 위치와 너무 가까우면 재시도
            if (Vector3.Distance(vendigo.transform.position, hit.position) < 1f)
            {
